...
@section('main')
            ...
            <div class="card shadow">
                ...
                <div class="card-body">
                    <table class="table table-striped table-bordered table-hover text-center">
                        ...
                        <tbody>
                            @foreach ($data_order as $od)
                                <tr>
                                    <form action="{{route('order.update',$od->id)}}" method="post">
                                    @csrf @method('PUT')
                                        <input type="hidden" name="note" value="{{$od->note}}">
                                        <input type="hidden" name="id_customer" value="{{$od->id_customer}}">
                                        <td scope="row">
                                            <input type="hidden" name="name" value="{{$od->name}}">
                                            {{ $od->name }}
                                        </td>
                                        <td>
                                            <input type="hidden" name="email" value="{{$od->email}}">
                                            {{ $od->email }}
                                        </td>
                                        <td>
                                            <input type="hidden" name="phone" value="{{$od->phone}}">
                                            {{ $od->phone }}
                                        </td>
                                        <td>
                                            <input type="hidden" name="address" value="{{$od->address}}">
                                            {{ $od->address }}
                                        </td>
                                        <td>{{ $od->created_at->format('d-m-Y') }}</td>
                                        <td>
                                            <select class="form-control form-control-sm text-dark" name="status" id="status">
                                                <option class="text-info" value="0" @if ($od->status==0) selected @endif>
                                                    <span class="">Awaiting Fulfillment</span>
                                                </option>
                                                <option class="text-warning" value="1" @if ($od->status==1) selected @endif>
                                                    <span class="">Awaiting Shipment</span>
                                                </option>
                                                <option class="text-success" value="2" @if ($od->status==2) selected @endif>
                                                    <span class="">Compelete</span>
                                                </option>
                                                <option class="text-danger" value="3" @if ($od->status==3) selected @endif>
                                        ...
                                        <td>
                                            ...
                                            <button type="button" class="btn btn-info" data-toggle="modal" data-target="#view_detail_{{$od->id}}"><i class="fa fa-search" aria-hidden="true"></i></button>
                                            
                                            <a href="{{ route('order.destroy', $od->id) }}"
                ...
                <div class="card-footer text-muted">
                    <div class="row">
                        <div class="col-5">
                            <h6>Showing 1 to 10 of {{ $data_order->count() }} entries</h6>
                        </div>
                        <div class="col-7">
                            {{ $data_order->appends(request()->all())->links() }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    @foreach ($data_order as $view_od)
        <!-- Modal -->
     <div class="modal fade" id="view_detail_{{$view_od->id}}" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
