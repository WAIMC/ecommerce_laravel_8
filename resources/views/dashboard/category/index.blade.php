            ...
            <div class="card shadow">
                ...
                <div class="card-body">
                    <table class="table table-striped table-bordered table-hover text-center">
                        ...
                        <tbody>
                            @foreach ($data_category as $cats)
                                <tr>
                                    <td scope="row">{{ $cats->id }}</td>
                                    <td>{{ $cats->name }}</td>
                                    <td>{{ $cats->category_product->count() }}</td>
                                    <td>{{ $cats->slug }}</td>
                                    <td>{{ $cats->parent_id }}</td>
                                    <td>{{ $cats->created_at->format('d-m-Y') }}</td>
                                    <td>
                                        <a href="{{ route('category.edit', $cats->id) }}" class="btn btn-info">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="{{ route('category.destroy', $cats->id) }}"
                ...
                <div class="card-footer text-muted">
                    <div class="row">
                        <div class="col-5">
                            <h6>Showing 1 to 10 of {{ $data_category->count() }} entries</h6>
                        </div>
                        <div class="col-7">
                            {{ $data_category->appends(request()->all())->links() }}
                        </div>
                    </div>
                </div>
            </div>
